...
型別和作用在它上面定義的方法必須在同一個包裡定義，這就是為什麼不能在 `int`、`float32(64)` 或類似這些的型別上定義方法。試圖在 `int` 型別上定義方法會得到一個編譯錯誤：
